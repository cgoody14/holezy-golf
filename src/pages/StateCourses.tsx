...
import { getStateBySlug, STATE_SEO_DATA } from '@/data/seoContent';

const CourseMap = lazy(() => import('@/components/CourseMap'));

// Geocode utilities (shared)
...
const StateCourses = () => {
  ...
  const geocodeCourses = useCallback(async (coursesToGeocode: Course[], stateLat: number, stateLng: number) => {
    ...
    const initial: GeocodedCourse[] = normalized.map((course, index) => {
      ...
      return { ...course, lat: stateLat + (seededRandom(seed) - 0.5) * 0.8, lng: stateLng + (seededRandom(seed + 1000) - 0.5) * 1.0 };
    });
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setIsLoading(true);
      try {
        let query = supabase.from('Course_Database').select('"Course Name", "Address", "Facility ID", "Phone", "Course Website"').not('"Course Name"', 'is', null).ilike('"Address"', `%${stateData.name}%`);
        ...
        setCourses(cd);
        geocodeCourses(cd, stateData.lat, stateData.lng);
      } catch (e) { console.error(e); setCourses([]); setGeocodedCourses([]); }
      finally { setIsLoading(false); }
    };
    load();
  }, [stateData, courseSearchTerm, geocodeCourses]);

  if (!stateData) {
    // Redirect to /courses for unknown states
  ...
  const handleBookCourse = () => {
    if (selectedCourse) {
      navigate('/booking', { state: { preselectedState: stateData.name, preselectedCourse: selectedCourse["Course Name"], preselectedFacilityId: selectedCourse["Facility ID"] } });
    }
  };
  ...
  const stateStructuredData = {
    ...
    "url": `https://holezygolf.com/golf-courses/${stateData.slug}`,
    "breadcrumb": {
      "@type": "BreadcrumbList",
      "itemListElement": [
        { "@type": "ListItem", "position": 1, "name": "Home", "item": "https://holezygolf.com" },
        { "@type": "ListItem", "position": 2, "name": stateData.name, "item": `https://holezygolf.com/golf-courses/${stateData.slug}` }
      ]
    }
  };
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      <SEOHead
        ...
        description={stateData.metaDescription}
        canonicalUrl={`/golf-courses/${stateData.slug}`}
        structuredData={stateStructuredData}
      />
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <h1 className="text-3xl md:text-4xl font-bold mb-4">{stateData.h1}</h1>
        <p className="text-lg text-muted-foreground mb-8">
          Explore golf courses in {stateData.name} and book your next tee time with Holezy Golf.
        </p>
        ...
        <div className="space-y-6 mb-12">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
            <Button variant="outline" onClick={() => navigate('/courses')} className="shrink-0">← Back to States</Button>
            ...
            <Card className="lg:col-span-2 golf-card-shadow overflow-hidden order-first lg:order-last">
              <div className="h-[300px] sm:h-[400px] lg:h-[540px]">
                <Suspense fallback={<MapLoader />}>
                  <CourseMap center={[stateData.lat, stateData.lng]} courses={safeGeocodedCourses} stateCode={stateData.code} selectedCourse={selectedCourse} onMarkerClick={handleMarkerClick} />
                </Suspense>
              </div>
            </Card>
            <Card className="lg:col-span-1 golf-card-shadow">
              <CardHeader className="pb-2"><CardTitle className="text-lg">Courses</CardTitle></CardHeader>
        ...
        <section className="max-w-4xl mx-auto mb-12">
          <h2 className="text-2xl font-bold mb-4">Explore Golf in {stateData.name}</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {stateData.cities.map(city => (
              <Link key={city.slug} to={`/golf-courses/${stateData.slug}/${city.slug}`}
                className="block p-4 rounded-lg border bg-card hover:bg-muted/50 transition-colors">
                <h3 className="font-semibold text-primary">{city.name} Golf Courses</h3>
                <p className="text-sm text-muted-foreground mt-1">Find and book tee times at the best courses in {city.name}.</p>
              </Link>
            ))}
            <Link to="/book" className="block p-4 rounded-lg border bg-primary/5 border-primary/20 hover:bg-primary/10 transition-colors">
              ...
              <p className="text-sm text-muted-foreground mt-1">Let our AI concierge secure your next round in {stateData.name}.</p>
            </Link>
          </div>
        </section>
      </div>
