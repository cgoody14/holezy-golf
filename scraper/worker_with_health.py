...
@app.route('/health')
def health():
    return "Holzey Golf Worker is running!", 200
...
def check_and_run_bookings():
    ...
    try:
        print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Checking for pending bookings...")
        ...
        print(f"Found {len(bookings)} pending booking(s)")
...
def execute_booking(booking, course_info):
    ...
    booking_id = booking.get("id")
    try:
        # Mark as processing so we don’t double-fire
...
def run_scheduler():
    """Run the booking scheduler"""
    print("🏌️ Holzey Golf Booking Worker Started!")
    ...
    # Poll every 2 minutes for new bookings
    schedule.every(2).minutes.do(check_and_run_bookings)
