-- Retry with proper regclass resolution for quoted identifier
DO $$
DECLARE tbl regclass := to_regclass('public."Client_Accounts"');
BEGIN
  IF tbl IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conrelid = tbl AND contype = 'p'
  ) THEN
    EXECUTE 'ALTER TABLE public."Client_Accounts" ADD PRIMARY KEY (id)';
  END IF;
END$$;
